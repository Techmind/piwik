...
class Test_Piwik_Integration_UIIntegrationTest extends IntegrationTestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        } else if (!self::isSlimerJsAvailable()) {
        ...
        // run slimerjs w/ all urls so we only invoke slimerjs once
        $urls = array();
...
return array(array('visitors_overview', "?$urlBase#$generalParams&module=VisitsSummary&action=index"));
        return array(
            // dashboard
    ...
    private static function runSlimerJs($urlInfo)
    {
        file_put_contents(PIWIK_INCLUDE_PATH . '/tmp/urls.txt', json_encode($urlInfo));
        $cmd = "slimerjs \"".PIWIK_INCLUDE_PATH."/tests/resources/slimerjs/capture.js\" 2>&1";
        ...
        if ($result !== 0
            ...
            echo "slimerjs failed: " . $output . "\n\ncommand used: $cmd\n";
            throw new Exception("slimerjs failed");
        }echo "OUT: $output\n";
        return $output;
    }
    ...
    {
        exec("slimerjs --help 2>&1", $output, $result);
